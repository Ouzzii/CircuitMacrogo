...
func (a *App) Detect_tex_distros(w http.ResponseWriter, r *http.Request) {
	//last distro check
	...
	if runtime.GOOS == "linux" {
		...
		if err != nil {
			...
			json.NewEncoder(w).Encode(a.Configuration)
			return
		}
...
func (a *App) Boxdims_is_installed(w http.ResponseWriter, r *http.Request) {
	UpdateEnv()
	...
	a.Configuration.WriteConf()
	json.NewEncoder(w).Encode(boxdims)
}
...
// popup ekraninda secilen distro
func (a *App) ChooseDistro(w http.ResponseWriter, r *http.Request) {
	var distro string
	if err := json.NewDecoder(r.Body).Decode(&distro); err != nil {
